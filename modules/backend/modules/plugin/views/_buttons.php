...
//'code'=>$code, 'id'=>$plugin->id_plugin
  ...
  if ($showSettingButton) {
    echo CHtml::link('<i class="glyphicon glyphicon-cog icon-white"></i> Настройки', $link, array('class' => 'btn btn-primary')).' ';
  }
  ...
  echo CHtml::ajaxLink('<i class="glyphicon glyphicon-off icon-white"></i> Отключить', Yii::app()->createUrl('backend/plugin/plugin/turnOff', array('code'=>$code)), array(
      'type' => 'POST',
      'dataType' => 'json',
  ...
  echo CHtml::ajaxLink('<i class="glyphicon glyphicon-check icon-white"></i> Включить', Yii::app()->createUrl('backend/plugin/plugin/turnOn', array('code'=>$code)), array(
      'type' => 'POST',
      'dataType' => 'json',
      'success' => 'function(data){if (data.error !== undefined) {alert(data.error); return;} jQuery("#plugin_buttons_'.$id.'").html(data.html); if (data.updateMenu === true) {$("#menu-side-main").daUpdateMenu({"idObject":'.$idObjectStr.', "idObjectView":'.$idObjectViewStr.', "url":"'.$url.'"});} }',
  ), array(
  ...
  echo CHtml::ajaxLink('<i class="glyphicon glyphicon-remove icon-white"></i> Удалить', Yii::app()->createUrl('backend/plugin/plugin/delete', array('code'=>$code)), array(
      'type' => 'POST',
      'dataType' => 'script',
