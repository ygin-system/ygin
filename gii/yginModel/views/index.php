...
$('#{$class}_connectionId').change(function(){
	...
	$.ajax({
		url: '".Yii::app()->getUrlManager()->createUrl('gii/yginModel/getTableNames')."',
		data: {db: this.value},
		dataType: 'json'
	}).done(function(data){
...
$('#{$class}_tableName').bind('keyup change', function(){
	...
	if(tableName.substring(tableName.length-1)!='*') {
		$('.form .row.model-class, .form .row.model-behavior').show();
	}
	else {
		$('#{$class}_modelClass').val('');
		$('.form .row.model-class, .form .row.model-behavior').hide();
	}
	...
	//скидываем значения behavior
	$('#{$class}_tsBehavior').prop('checked', false).triggerHandler('click');
	
});
$('.form .row.model-class').toggle($('#{$class}_tableName').val().substring($('#{$class}_tableName').val().length-1)!='*');

$('#{$class}_tsBehavior').on('click', function(e) {
  var el = $(this);
  if (el.prop('checked')) {
    $('.model-behavior .tsbehavior-settings').show();
    $.ajax({
      url: '".Yii::app()->getUrlManager()->createUrl('gii/yginModel/getDateAttributes')."',
      data: {db: $('#{$class}_connectionId').val(), tableName: $('#{$class}_tableName').val()},
      dataType: 'json'
    }).done(function(data) {
      var caOptions = uaOptions = '';
      if (data.length) {
        for(var i = 0; i < data.length; i++) {
          caOptions += '<option value=\''+data[i]+'\'>'+data[i]+'</option>';
        }
        uaOptions = '<option value=\'\'></option>'+caOptions;
      }
      $('#{$class}_tsBehaviorCreateAttribute').html(caOptions);
      $('#{$class}_tsBehaviorUpdateAttribute').html(uaOptions);
    });
  } else {
     $('.model-behavior .tsbehavior-settings').hide();
     $('#{$class}_tsBehaviorCreateAttribute').html('');
     $('#{$class}_tsBehaviorUpdateAttribute').html('');
  }
});
...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row model-behavior">
		<?php echo $form->labelEx($model,'tsBehavior'); ?>
		<?php echo $form->checkBox($model,'tsBehavior'); ?>
		<div class="clr"></div>
		<div class="tsbehavior-settings" style="display:none; overflow:hidden">
  		<div style="float:left;">
  		<?php echo $form->labelEx($model,'tsBehaviorCreateAttribute'); ?>
  		<?php echo $form->dropDownList($model,'tsBehaviorCreateAttribute', array()); ?>
  		</div>
  		<div style="float:left; margin-left: 20px;">
  		<?php echo $form->labelEx($model,'tsBehaviorUpdateAttribute'); ?>
  		<?php echo $form->dropDownList($model,'tsBehaviorUpdateAttribute', array()); ?>
  		</div>
		</div>
		<div class="tooltip">
			CTimestampBehavior will automatically fill date and time related attributes
			when the active record is created and/or upadated.
    ...
    public function behaviors(){
		...
		<?php echo $form->error($model,'timestampBehavior'); ?>
	</div>
