...
class ExtendedClientScript extends CClientScript
{
  ...
  */
  ...
  private $dependResource = array();
  
  public function addDependResource($mainFile, $dependFile) {
    $dependArray = array();
    if (is_array($dependFile)) {
      foreach ($dependFile AS $k => $v) {
        if (is_int($k)) {
          $k = $v;
          $v = '';
        }
      ...
      $dependArray[$dependFile] = '';
    }
    
    if (!isset($this->dependResource[$mainFile])) {
      $this->dependResource[$mainFile] = $dependArray;
    } else {
      $this->dependResource[$mainFile] = array_merge($this->dependResource[$mainFile], $dependArray);
    }
  }
...
//            $fileContent = str_replace($addPath.$dependFileName, $this->fileUrl.$relativePath, $fileContent);
        ...
        if ($type == 'css' && strpos($fn, ".min") !== false) {  // mixa, почему-то уже сжатые css-файлы жмутся с ошибками (bootstrap)
          $notCompress .= $fileContent;
        } else {
          $combinedFile .= $fileContent;
          if ($type == 'js') $combinedFile .= ";\n\r";
        }
      }
